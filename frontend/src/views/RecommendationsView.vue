<template>
  <ErrorBoundary context="AI Show Recommendations" fallback-message="Unable to load AI recommendations">
    <v-container class="recommendations-container">
      <div class="recommendations-header">
        <div class="header-content">
          ...
          <p class="page-subtitle">Get personalized show suggestions powered by AI</p>
        </div>
        <div class="ai-badge">
      ...
      <div v-if="shows.length > 0" class="recommendations-results">
        <div class="section-header">
          <h2 class="section-title">Your AI Recommendations</h2>
          <v-chip color="success" variant="outlined">{{ shows.length }} shows</v-chip>
        </div>
      ...
      <!-- Empty State -->
      <div v-else-if="hasSearched && shows.length === 0" class="empty-state">
      ...
      <div v-else class="getting-started">
        <v-card class="text-center pa-8" variant="outlined">
          ...
          <h3 class="mb-4">Ready to discover new shows?</h3>
          <p class="mb-4">Set your preferences above and let our AI recommend shows tailored just for you!</p>
...
import type { ShowPreview } from "../types";
...
import { AI_MAX_RATING, AI_MAX_RECOMMENDATIONS, AI_MIN_RATING, AI_MIN_RECOMMENDATIONS, SHOW_GENRES } from "../const";
import { getUrl } from "../helpers";
import { $toast } from "../toast";

type ShowGenre = (typeof SHOW_GENRES)[number];

interface RecommendationPreferences {
  preferredGenre: ShowGenre | null;
  recommendationsNumber: number;
  minRating: number | undefined;
}

const shows: Ref<ShowPreview[]> = ref([]);
const hasSearched = ref(false);

// Error handling composable
const recommendationsOperation = useApiCall("AI Show Recommendations");
...
async function getRecommendations(): Promise<void> {
  ...
  const result = await recommendationsOperation.execute(async () => {
    ...
    return response.data as ShowPreview[];
  });
  ...
  if (result.success && result.data) {
    ...
    if (shows.value.length === 0) {
      ...
      $toast.success(`Found ${shows.value.length} AI recommendations for you!`);
    }
  } else {
    shows.value = [];
  }
