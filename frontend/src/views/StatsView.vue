<template>
  <ErrorBoundary context="Show Statistics" fallback-message="Unable to load your statistics">
    <div class="stats-view">
      <div class="page-header">
        <div class="header-content">
          <h1 class="page-title">Your Show Stats</h1>
          <p class="page-subtitle">Insights into your show watching journey</p>
        </div>
      </div>
          ...
          <v-card class="year-review-card">
            <v-card-title class="year-review-title">
              ...
              <div>
                ...
                <p class="text-subtitle-1 mb-0 mt-1">Your show watching journey this year</p>
              </div>
            </v-card-title>
            ...
            <v-card-text>
              ...
              <v-row class="mb-4">
                <v-col cols="12" sm="6" md="3">
                  <div class="yearly-stat-item">
                    <v-icon size="40" color="success" class="mb-2">mdi-television</v-icon>
                    <h3 class="text-h4 font-weight-bold">{{ stats.yearlyOverview.totalShowsWatched }}</h3>
                    <p class="text-body-2">Shows Watched</p>
                ...
                <v-col cols="12" sm="6" md="3">
                  <div class="yearly-stat-item">
                    ...
                    <p class="text-caption text-muted">{{ stats.yearlyOverview.peakMonthCount }} shows</p>
                  </div>
                </v-col>
              ...
              <div v-if="stats.yearlyMilestones" class="milestones-section">
                ...
                <v-row>
                  <v-col v-if="stats.yearlyMilestones.firstShow" cols="12" md="6">
                    <v-card variant="outlined" class="milestone-card">
                      <v-card-text>
                        <div class="d-flex align-center mb-2">
                          <v-icon color="green" class="mr-2">mdi-play</v-icon>
                          <span class="font-weight-medium">First Show</span>
                        </div>
                        <p class="text-body-1 mb-1">{{ stats.yearlyMilestones.firstShow.title }}</p>
                        <p class="text-caption text-muted">{{ stats.yearlyMilestones.firstShow.date }}</p>
                      </v-card-text>
                    </v-card>
                  </v-col>

                  <v-col v-if="stats.yearlyMilestones.highestRatedShow" cols="12" md="6">
                    <v-card variant="outlined" class="milestone-card">
                      <v-card-text>
                        <div class="d-flex align-center mb-2">
                        ...
                        <p class="text-body-1 mb-1">{{ stats.yearlyMilestones.highestRatedShow.title }}</p>
                        <div class="d-flex align-center">
                          <v-rating
                            :model-value="stats.yearlyMilestones.highestRatedShow.rating"
                            readonly
                            density="compact"
                            size="small"
                            color="amber"
                          ...
                          <span class="ml-2 text-caption"
                            >{{ stats.yearlyMilestones.highestRatedShow.rating }}/10</span
                          >
                        </div>
                      </v-card-text>
                    </v-card>
                  ...
                  <v-col v-if="stats.yearlyMilestones.topGenre" cols="12" md="6">
                    <v-card variant="outlined" class="milestone-card">
                      <v-card-text>
                        ...
                        <p class="text-caption text-muted">{{ stats.yearlyMilestones.topGenre.count }} shows</p>
                      </v-card-text>
                    </v-card>
                  </v-col>
                </v-row>
            ...
            <v-col cols="12" sm="6" md="3">
              <v-card class="stat-card text-center">
                <v-card-text>
                  ...
                  <h2 class="text-h4 font-weight-bold">{{ stats.totalShowsToWatch }}</h2>
          ...
          <v-row>
            ...
            <v-col cols="12" md="6">
              <v-card class="stat-card">
                ...
                <v-card-text>
                  <div v-if="!stats.oldestShow && !stats.newestShow" class="text-center">
                    <p class="text-muted">No air date data available</p>
                  </div>
                  <div v-else class="release-overview">
                    <div v-if="stats.averageFirstAirYear" class="overview-item mb-4">
                      <div class="d-flex justify-space-between align-center mb-2">
                        <div class="d-flex align-center">
                          <v-icon size="20" color="primary" class="mr-2">mdi-calendar-star</v-icon>
                          <span class="font-weight-medium">Average First Air Year</span>
                        </div>
                        <v-chip size="small" color="primary">{{ Math.round(stats.averageFirstAirYear) }}</v-chip>
                      </div>
                    </div>

                    <div v-if="stats.oldestShow" class="overview-item mb-3">
                      <div class="d-flex align-center mb-1">
                        <v-icon size="16" color="brown" class="mr-2">mdi-history</v-icon>
                        <span class="text-caption font-weight-medium">Oldest Show</span>
                      </div>
                      <div class="ml-6">
                        <p class="text-body-2 mb-0">{{ stats.oldestShow.title }}</p>
                        <p class="text-caption text-muted">{{ stats.oldestShow.firstAirYear }}</p>
                      </div>
                    </div>

                    <div v-if="stats.newestShow" class="overview-item">
                      <div class="d-flex align-center mb-1">
                        <v-icon size="16" color="green" class="mr-2">mdi-new-box</v-icon>
                        <span class="text-caption font-weight-medium">Newest Show</span>
                      </div>
                      <div class="ml-6">
                        <p class="text-body-2 mb-0">{{ stats.newestShow.title }}</p>
                        <p class="text-caption text-muted">{{ stats.newestShow.firstAirYear }}</p>
                      </div>
                    </div>
                  </div>
                </v-card-text>
...
interface YearlyMilestones {
  firstShow?: ShowMilestone;
  lastShow?: ShowMilestone;
  highestRatedShow?: ShowMilestone;
  topGenre?: TopItemMilestone;
  topActor?: TopItemMilestone;
}

interface AirDateShow {
  title: string;
  firstAirDate: string;
  firstAirYear: number;
}

interface FirstAirYearItem {
  year: number;
  count: number;
}
...
interface Stats {
  ...
  averageFirstAirYear: number | null;
  oldestShow: AirDateShow | null;
  newestShow: AirDateShow | null;
  topFirstAirYears: FirstAirYearItem[];
...
function getVintagePercentage(count: number): number {
  const total = stats.value.totalShowsWatched;
